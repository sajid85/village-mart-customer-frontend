  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const initialImageStates = filteredData.reduce((acc: {[key: string]: ImageState}, product: Product) => {
        ...
        const wishlistPromises = filteredData.map((product: Product) => isInWishlist(product.id));
        const wishlistStatuses = await Promise.all(wishlistPromises);
        const wishlistSet = new Set(
          filteredData.filter((_: Product, index: number) => wishlistStatuses[index]).map((product: Product) => product.id)
        );
